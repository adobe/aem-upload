...
class HttpRequest extends UploadBase {
     ...
     * sent.
     *
     * @param {string|object|ArrayBuffer|ArrayBufferView|URLSearchParams|FormData|File|Blob|Stream|Buffer} rawData
     *  Data to send in the request's body.
     * @param {number} [dataSize] The total size, in bytes, of the provided data. If not provided then
     *  the request will attempt to derive the size from the data, but note that this may fail for
     *  some data types.
     * @returns The current request instance, so that methods can be chained.
     */
