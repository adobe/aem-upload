...
function cancelFromController(cancelEventData) {
    const { fileName } = cancelEventData;

    if (fileName) {
        // use fileName as cancel ID and cancel any matches
        this.cancel(fileName);
    } else {
        // no fileName, cancel ALL requests
        this.cancelAll();
    }
}
...
export default class HttpClient extends UploadOptionsBase {
    ...
    async submit(httpRequest, httpResult = false) {
        ...
        try {
            return new HttpResponse(this.getOptions(), await timedRequest(httpRequest.toJSON(), getRetryOptions(this.getUploadOptions(), httpResult), cancelToken));
        } catch (e) {
