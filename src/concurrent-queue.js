...
/**
 * Creates a new "batch" that has a given number of items in it. When all
 * items in the batch have been processed, the given callback will be invoked.
 * @param {number} totalCount Total number of items in the batch.
 * @param {function} callback Invoked without arguments when all items in the
 *  batch have processed.
 * @returns {string} The ID of the new batch.
 */
function createBatch(totalCount, callback) {
    const batchId = uuid();
...
/**
 * Increments the total number of items that have been processed in a batch.
 * If all items in the batch are processed, the method will also invoke the
 * batch's callback.
 * @param {string} batchId Batch whose processed count will be incremented.
 */
...
export default class ConcurrentQueue extends UploadOptionsBase {
     ...
     *  be processed. The function is expected to return a promise. It will be
    ...
    push(item, callback) {
        return new Promise((res) => {
            this.queue.push({
                data: item,
                batch: createBatch.call(this, 1, res),
                worker: callback,
            });
        });
    }
     ...
     * @param {function} callback Will be invoked when one of item is ready to
     *  be processed. The function is expected to return a promise. It will be
     *  invoked with a single argument: the item to be processed.
     * @returns {Promise} Will be resolved when all provided items have been processed.
     */
    pushAll(items, callback) {
        return new Promise((res) => {
            const batch = createBatch.call(this, items.length, res);
            items.forEach((item) => {
                this.queue.push({
                    data: item,
                    batch,
